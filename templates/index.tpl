...
{block name="page"}
	...
	<div id="importExportTabs">
		...
		<div id="settings-tab">
			{$ftpWarning}
			{if !$allowExport}
				<div class="pkp_notification" id="porticoConfigurationErrors">
					{foreach from=$configurationErrors item=configurationError}
						{if $configurationError == \APP\plugins\PubObjectsExportPlugin::EXPORT_CONFIG_ERROR_SETTINGS}
							{include file="controllers/notification/inPlaceNotificationContent.tpl" notificationId=porticoConfigurationErrors notificationStyleClass="notifyWarning" notificationTitle="plugins.importexport.common.missingRequirements"|translate notificationContents="plugins.importexport.common.error.pluginNotConfigured"|translate}
						{/if}
					{/foreach}
				</div>
			{/if}
			{capture assign=porticoSettingsGridUrl}{url router=PKP\core\PKPApplication::ROUTE_COMPONENT component="grid.settings.plugins.settingsPluginGridHandler" op="manage" plugin="PorticoExportPlugin" category="importexport" verb="index" escape=false}{/capture}
				...
				<div id="exportPublications-tab">
					...
					<form id="exportPublicationXmlForm" class="pkp_form" action="{plugin_url path="exportPublications"}" method="post">
						...
						{fbvFormArea id="publicationsXmlForm"}
							{capture assign=publicationsListGridUrl}{url router=PKP\core\PKPApplication::ROUTE_COMPONENT component="grid.publications.ExportPublishedPublicationsListGridHandler" op="fetchGrid" plugin="PorticoExportPlugin" category="importexport" escape=false}{/capture}
				...
				<div id="exportSubmissions-tab">
					...
					<form id="exportSubmissionXmlForm" class="pkp_form" action="{plugin_url path="exportSubmissions"}" method="post">
						...
						{fbvFormArea id="submissionsXmlForm"}
							{capture assign=submissionsListGridUrl}{url router=PKP\core\PKPApplication::ROUTE_COMPONENT component="grid.submissions.ExportPublishedSubmissionsListGridHandler" op="fetchGrid" plugin="PorticoExportPlugin" category="importexport" escape=false}{/capture}
